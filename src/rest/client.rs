...
impl BitFlyerRest {
    ...
    pub fn builder() -> BitFlyerRestBuilder {
        BitFlyerRestBuilder::default()
    }
    ...
    async fn handle_response<T: DeserializeOwned>(&self, resp: Response) -> T {
        ...
        if status.is_success() {
            ...
            match from_str::<BitFlyerErrorResponse>(&content) {
                Ok(ret) => throw!(BitFlyerError::from(ret.error)),
                Err(e) => {
